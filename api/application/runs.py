...
from api.domain.models import Project, Run, RunEvent, RunKind, RunOptions, RunStatus
...
class GenerationService:
    ...
    async def execute(self, run: Run) -> Run:
        ...
        return await self._finalize(run, exit_code)
    ...
    def _write_work_dir_sync(work_dir: Path, txt: str, structure: dict[str, Any] | None) -> None:
        ...
        if structure is not None:
            (work_dir / book_command.BOOK_STRUCTURE_FILENAME).write_text(
    ...
    async def _finalize(self, run: Run, exit_code: int) -> Run:
