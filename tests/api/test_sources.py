...
async def test_attach_list_get_update_detach_round_trip(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "paper.pdf")
    ...
    list_response = await authed_client.get(f"/api/v1/projects/{project['id']}/sources")
    ...
    project_response = await authed_client.get(f"/api/v1/projects/{project['id']}")
    assert project_response.status_code == 200
    assert project_response.json()["sources"] == [source]

    summaries = await authed_client.get("/api/v1/projects")
    ...
    empty_list = await authed_client.get(f"/api/v1/projects/{project['id']}/sources")
    assert empty_list.json()["total"] == 0

    # Detaching a source does not delete the underlying file.
    file_get = await authed_client.get(f"/api/v1/files/{file['id']}")
    assert file_get.status_code == 200


async def _attach_source(authed_client: AsyncClient, project_id: str, filename: str) -> dict:
    file = await _upload_file(authed_client, filename)
    response = await authed_client.post(
...
async def test_list_sources_query_count_stays_flat_as_sources_grow(
    ...
    project = await _create_project(authed_client)
    await _attach_source(authed_client, project["id"], "one.txt")

    with count_statements() as statements:
        response = await authed_client.get(f"/api/v1/projects/{project['id']}/sources")
    assert response.status_code == 200
    assert response.json()["total"] == 1
    first_page_statement_count = len(statements)

    for i in range(9):
        await _attach_source(authed_client, project["id"], f"more-{i}.txt")
...
async def test_add_source_defaults_type_from_extension(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
...
async def test_add_source_explicit_type_overrides_inference(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "book.pdf")
...
async def test_add_source_rejects_implausible_year(authed_client: AsyncClient) -> None:
    """Regression for issue #61: `year="banana"` (and other non-year
    strings) used to be accepted with no validation at all."""
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "paper.pdf")

    not_a_year = await authed_client.post(
        f"/api/v1/projects/{project['id']}/sources",
        json={"fileId": file["id"], "year": "banana"},
    )
    out_of_range = await authed_client.post(
        f"/api/v1/projects/{project['id']}/sources",
...
async def test_update_source_rejects_implausible_year(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "paper.pdf")
    source = (
...
async def test_update_source_type_persists(authed_client: AsyncClient) -> None:
    """Regression for issue #61: `SourceUpdate` had no `type` field at all,
    so a source's type could only ever be set at creation time."""
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "notes.md")
    source = (
        await authed_client.post(
            f"/api/v1/projects/{project['id']}/sources", json={"fileId": file["id"]}
        )
...
async def test_add_source_unknown_project_returns_404(authed_client: AsyncClient) -> None:
    file = await _upload_file(authed_client, "notes.txt")
...
async def test_add_source_unknown_file_returns_404(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
...
async def test_add_source_ineligible_file_returns_422(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "refs.bib")
...
async def test_add_duplicate_file_in_same_project_returns_409(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "book.pdf")
...
async def test_same_file_can_be_attached_to_different_projects(authed_client: AsyncClient) -> None:
    project_a = await _create_project(authed_client, title="A")
    project_b = await _create_project(authed_client, title="B")
    file = await _upload_file(authed_client, "book.pdf")
...
async def test_delete_file_referenced_by_source_returns_409(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "book.pdf")
    add_response = await authed_client.post(
        f"/api/v1/projects/{project['id']}/sources", json={"fileId": file["id"]}
    ...
    delete_response = await authed_client.delete(f"/api/v1/files/{file['id']}")

    assert delete_response.status_code == 409
    assert delete_response.headers["content-type"] == "application/problem+json"


async def test_deleting_project_cascades_to_sources(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "book.pdf")
    add_response = await authed_client.post(
        f"/api/v1/projects/{project['id']}/sources", json={"fileId": file["id"]}
    ...
    delete_response = await authed_client.delete(f"/api/v1/projects/{project['id']}")
    assert delete_response.status_code == 204
    ...
    file_get = await authed_client.get(f"/api/v1/files/{file['id']}")
    assert file_get.status_code == 200
    file_delete = await authed_client.delete(f"/api/v1/files/{file['id']}")
    assert file_delete.status_code == 204


async def test_get_update_delete_missing_source_return_404(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
...
async def test_sources_endpoints_404_for_unknown_project(authed_client: AsyncClient) -> None:
    missing_project_id = uuid.uuid4()

    response = await authed_client.get(f"/api/v1/projects/{missing_project_id}/sources")
...
async def test_duplicate_project_copies_sources_pointing_at_same_files(
    ...
    original = await authed_client.get(f"/api/v1/projects/{project['id']}")
...
async def test_duplicate_project_does_not_copy_soft_deleted_sources(
    ...
    await authed_client.delete(f"/api/v1/projects/{project['id']}/sources/{source_id}")
...
async def test_removed_source_row_is_soft_deleted_not_erased(
    ...
    # Gone from the API...
    assert (
        await authed_client.get(f"/api/v1/projects/{project['id']}/sources/{source_id}")
    ).status_code == 404
    list_response = await authed_client.get(f"/api/v1/projects/{project['id']}/sources")
...
async def test_reattaching_file_after_soft_delete_succeeds(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    file = await _upload_file(authed_client, "book.pdf")
...
async def test_file_with_only_soft_deleted_source_is_no_longer_referenced(
    ...
    still_referenced = await authed_client.delete(f"/api/v1/files/{file['id']}")
