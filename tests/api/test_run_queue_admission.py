...
def test_admission_allows_a_full_run_with_an_empty_lane() -> None:
    assert queue_admission_blocker(RunKind.full, [], cap=5) is None
...
def test_admission_allows_a_full_run_behind_a_queued_regenerate() -> None:
    ...
    assert queue_admission_blocker(RunKind.full, lane, cap=5) is None
...
def test_admission_allows_chained_regenerates_behind_each_other() -> None:
    ...
    assert queue_admission_blocker(RunKind.regenerate_section, lane, cap=5) is None
    assert queue_admission_blocker(RunKind.export, lane, cap=5) is None


def test_admission_blocks_regenerate_behind_a_queued_full_run() -> None:
    lane = [_lane_run(RunKind.full, RunStatus.queued)]
    blocker = queue_admission_blocker(RunKind.regenerate_section, lane, cap=5)
...
def test_admission_blocks_export_behind_a_running_full_run() -> None:
    ...
    blocker = queue_admission_blocker(RunKind.export, lane, cap=5)
    assert blocker is not None
    assert "full run" in blocker


def test_admission_blocks_retry_behind_a_queued_full_run() -> None:
    # `RunService.retry` creates a `kind == full` run, so this exercises the
    # same "full blocks non-full" rule from `retry`'s own call site.
    lane = [_lane_run(RunKind.full, RunStatus.queued)]
    assert queue_admission_blocker(RunKind.full, lane, cap=5) is None  # retry is itself `full`
...
def test_admission_blocks_once_the_cap_is_reached() -> None:
    ...
    blocker = queue_admission_blocker(RunKind.regenerate_section, lane, cap=3)
    assert blocker is not None
    assert "queue is full" in blocker
    assert queue_admission_blocker(RunKind.regenerate_section, lane, cap=4) is None
