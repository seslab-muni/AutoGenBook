...
async def test_create_chapter_and_subsections_yields_expected_numbers_and_keys(
    ...
    chapter1 = await _create_node(authed_client, project_id, title="Chapter 1")
    chapter2 = await _create_node(authed_client, project_id, title="Chapter 2")
    section11 = await _create_node(
        authed_client, project_id, title="Section 1.1", parentId=chapter1["id"]
    )
    section12 = await _create_node(
        authed_client, project_id, title="Section 1.2", parentId=chapter1["id"]
    )
...
async def test_create_node_applies_defaults(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client, equationFrequencyLevel=2)

    node = await _create_node(authed_client, project["id"], title="Chapter 1")
...
async def test_create_node_appends_order_index_by_default(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    first = await _create_node(authed_client, project["id"], title="First")
    second = await _create_node(authed_client, project["id"], title="Second")
...
async def test_create_node_with_explicit_order_index_shifts_existing_siblings(
    ...
    project = await _create_project(authed_client)
    project_id = project["id"]
    parent = await _create_node(authed_client, project_id, title="Parent")
    first = await _create_node(authed_client, project_id, title="First", parentId=parent["id"])
    second = await _create_node(authed_client, project_id, title="Second", parentId=parent["id"])
    ...
    list_response = await authed_client.get(f"/api/v1/projects/{project_id}/outline")
...
async def test_create_node_with_explicit_order_index_at_root_never_duplicates_positions(
    ...
    project = await _create_project(authed_client)
    project_id = project["id"]
    first = await _create_node(authed_client, project_id, title="First")
    second = await _create_node(authed_client, project_id, title="Second")
    assert (first["orderIndex"], second["orderIndex"]) == (0, 1)
...
async def test_create_node_rejects_out_of_range_order_index_with_422(
    ...
    project = await _create_project(authed_client)
    project_id = project["id"]
    await _create_node(authed_client, project_id, title="Only")
    ...
    negative_response = await authed_client.post(
        f"/api/v1/projects/{project_id}/outline",
...
async def test_create_node_depth_limit_rejected_with_422(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client, maxOutlineLevels=3)
    project_id = project["id"]

    level1 = await _create_node(authed_client, project_id, title="L1")
    level2 = await _create_node(authed_client, project_id, title="L2", parentId=level1["id"])
    level3 = await _create_node(authed_client, project_id, title="L3", parentId=level2["id"])
...
async def test_create_node_rejects_blank_title_with_422(authed_client: AsyncClient) -> None:
...
async def test_replace_outline_rejects_blank_node_title(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
...
async def test_create_node_missing_parent_404s(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
...
async def test_get_and_list_flat_and_tree(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    project_id = project["id"]
    chapter = await _create_node(authed_client, project_id, title="Chapter 1")
    section = await _create_node(authed_client, project_id, title="Section 1.1", parentId=chapter["id"])

    get_response = await authed_client.get(f"/api/v1/projects/{project_id}/outline/{section['id']}")
    assert get_response.status_code == 200
    assert get_response.json()["title"] == "Section 1.1"

    flat_response = await authed_client.get(f"/api/v1/projects/{project_id}/outline")
    ...
    tree_response = await authed_client.get(
...
async def test_get_node_404_is_problem_json(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
...
async def test_list_outline_404_when_project_missing(authed_client: AsyncClient) -> None:
    response = await authed_client.get(f"/api/v1/projects/{uuid.uuid4()}/outline")

    assert response.status_code == 404


async def test_patch_content_markdown_recomputes_actual_words(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    node = await _create_node(authed_client, project["id"], title="Chapter 1")
...
async def test_patch_rejects_server_derived_fields(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    node = await _create_node(authed_client, project["id"], title="Chapter 1")
...
async def test_patch_rename_and_status(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    node = await _create_node(authed_client, project["id"], title="Draft title")
...
async def test_patch_target_pages_recomputes_word_budget(authed_client: AsyncClient) -> None:
    ...
    project = await _create_project(authed_client)
    node = await _create_node(authed_client, project["id"], title="Chapter 1", targetPages=2)
...
async def test_patch_toggles_structure_locked(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    node = await _create_node(authed_client, project["id"], title="Chapter 1")
    assert node["structureLocked"] is True
...
async def test_move_node_between_parents_updates_positions(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    project_id = project["id"]
    chapter1 = await _create_node(authed_client, project_id, title="Chapter 1")
    chapter2 = await _create_node(authed_client, project_id, title="Chapter 2")
    section = await _create_node(
        authed_client, project_id, title="Section", parentId=chapter1["id"]
    )
...
async def test_move_node_rejects_move_into_own_subtree(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    project_id = project["id"]
    parent = await _create_node(authed_client, project_id, title="Parent")
    child = await _create_node(authed_client, project_id, title="Child", parentId=parent["id"])
...
async def test_move_node_rejects_move_exceeding_max_outline_levels(
    ...
    project = await _create_project(authed_client, maxOutlineLevels=2)
    project_id = project["id"]
    chapter_a = await _create_node(authed_client, project_id, title="A")
    chapter_b = await _create_node(authed_client, project_id, title="B")
    grandchild_host = await _create_node(
        authed_client, project_id, title="B.1", parentId=chapter_b["id"]
    )
...
async def test_reorder_within_same_parent(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    project_id = project["id"]
    first = await _create_node(authed_client, project_id, title="First")
    second = await _create_node(authed_client, project_id, title="Second")
...
async def test_update_node_404(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
...
async def test_delete_node_soft_deletes_subtree(
    ...
    chapter = await _create_node(authed_client, project_id, title="Chapter 1")
    section = await _create_node(
        authed_client, project_id, title="Section 1.1", parentId=chapter["id"]
    )
    other_chapter = await _create_node(authed_client, project_id, title="Chapter 2")
    ...
    get_deleted = await authed_client.get(
        f"/api/v1/projects/{project_id}/outline/{section['id']}"
    )
...
async def test_delete_node_404(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
...
async def test_outline_reads_survive_a_live_node_under_a_soft_deleted_parent(
    ...
    project_response = await authed_client.get(f"/api/v1/projects/{project_id}")
...
async def test_repository_add_rejects_a_node_under_a_soft_deleted_parent(
    ...
    chapter = await _create_node(authed_client, str(project_id), title="Chapter 1")
...
async def test_replace_outline_creates_full_tree_and_returns_flat(
    ...
    project = await _create_project(authed_client)
    project_id = project["id"]
    stale = await _create_node(authed_client, project_id, title="Stale node")
    ...
    response = await authed_client.put(f"/api/v1/projects/{project_id}/outline", json=body)
...
async def test_soft_deleted_siblings_do_not_affect_new_node_positions(
    ...
    project = await _create_project(authed_client)
    project_id = project["id"]
    await _create_node(authed_client, project_id, title="Chapter 1")
    chapter2 = await _create_node(authed_client, project_id, title="Chapter 2")
    await _create_node(authed_client, project_id, title="Chapter 3")
    ...
    await _create_node(authed_client, project_id, title="Chapter 4")
...
async def test_replace_outline_with_empty_list_reports_limit_at_least_one(
    ...
    response = await authed_client.put(f"/api/v1/projects/{project['id']}/outline", json=[])
...
async def test_replace_outline_404_when_project_missing(authed_client: AsyncClient) -> None:
    response = await authed_client.put(
...
async def test_project_response_embeds_outline_tree(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    chapter = await _create_node(authed_client, project["id"], title="Chapter 1")
    await _create_node(authed_client, project["id"], title="Section 1.1", parentId=chapter["id"])
...
async def test_project_summary_outline_node_count_is_real(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    await _create_node(authed_client, project["id"], title="Chapter 1")
    await _create_node(authed_client, project["id"], title="Chapter 2")
...
async def test_create_project_with_wizard_outline(authed_client: AsyncClient) -> None:
    payload = {
        **MINIMAL_PROJECT,
        "outline": [
            {
...
async def test_duplicate_project_deep_copies_outline_with_new_ids(
    ...
    project = await _create_project(authed_client, title="Original")
    chapter = await _create_node(authed_client, project["id"], title="Chapter 1")
    section = await _create_node(
        authed_client, project["id"], title="Section 1.1", parentId=chapter["id"]
    )
    ...
    original_response = await authed_client.get(f"/api/v1/projects/{project['id']}")
...
async def test_flat_and_tree_formats_are_equivalent(authed_client: AsyncClient) -> None:
    project = await _create_project(authed_client)
    project_id = project["id"]
    chapter1 = await _create_node(authed_client, project_id, title="Chapter 1")
    await _create_node(authed_client, project_id, title="Section 1.1", parentId=chapter1["id"])
    await _create_node(authed_client, project_id, title="Section 1.2", parentId=chapter1["id"])
    await _create_node(authed_client, project_id, title="Chapter 2")
