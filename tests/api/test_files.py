...
async def _upload(
    authed_client: AsyncClient, filename: str, content: bytes, content_type: str = "text/plain"
):
...
async def test_upload_list_get_download_delete_round_trip(
    authed_client: AsyncClient, file_storage: InMemoryFileStorage
) -> None:
    content = b"hello world, this is a test file"
    expected_sha256 = hashlib.sha256(content).hexdigest()

    upload_response = await _upload(authed_client, "notes.txt", content, "text/plain")
    ...
    list_response = await authed_client.get("/api/v1/files")
    ...
    get_response = await authed_client.get(f"/api/v1/files/{file_id}")
    assert get_response.status_code == 200
    assert get_response.json() == body

    download_response = await authed_client.get(f"/api/v1/files/{file_id}/content")
    ...
    delete_response = await authed_client.delete(f"/api/v1/files/{file_id}")
    assert delete_response.status_code == 204

    assert not await file_storage.exists(f"uploads/{file_id}/notes.txt")
    assert (await authed_client.get(f"/api/v1/files/{file_id}")).status_code == 404

    empty_list = await authed_client.get("/api/v1/files")
...
async def test_list_files_filters_by_kind(
    ...
    `kind` filter."""
    upload_response = await _upload(authed_client, "notes.txt", b"hello", "text/plain")
    assert upload_response.status_code == 201
    ...
    unfiltered = await authed_client.get("/api/v1/files")
    assert unfiltered.json()["total"] == 2

    uploads_only = await authed_client.get("/api/v1/files", params={"kind": "upload"})
    assert uploads_only.json()["total"] == 1
    assert [item["id"] for item in uploads_only.json()["items"]] == [upload_id]

    artifacts_only = await authed_client.get("/api/v1/files", params={"kind": "artifact"})
    assert artifacts_only.json()["total"] == 1
    assert artifacts_only.json()["items"][0]["filename"] == "book.md"


async def test_get_missing_file_returns_404(authed_client: AsyncClient) -> None:
    response = await authed_client.get("/api/v1/files/00000000-0000-0000-0000-000000000000")
    assert response.status_code == 404
    assert response.headers["content-type"] == "application/problem+json"


async def test_delete_missing_file_returns_404(authed_client: AsyncClient) -> None:
    response = await authed_client.delete("/api/v1/files/00000000-0000-0000-0000-000000000000")
    assert response.status_code == 404


async def test_kb_eligible_extensions(authed_client: AsyncClient) -> None:
    for filename, expected in [
        ...
        response = await _upload(
            authed_client, filename, b"content", "application/octet-stream"
        )
        assert response.status_code == 201
        assert response.json()["kbEligible"] is expected, filename


async def test_upload_falls_back_to_guessed_content_type(authed_client: AsyncClient) -> None:
    response = await _upload(authed_client, "diagram.md", b"# hi", "application/octet-stream")
    assert response.status_code == 201
    assert response.json()["contentType"] == "text/markdown"


async def test_upload_over_max_size_returns_413_and_leaves_no_object(
    authed_client: AsyncClient, app: FastAPI, file_storage: InMemoryFileStorage
) -> None:
    app.dependency_overrides[get_settings] = lambda: Settings(max_upload_mb=0)
    try:
        response = await _upload(authed_client, "big.txt", b"more than zero bytes")
    finally:
...
async def test_upload_over_max_size_rejected_by_content_length_never_touches_storage(
    authed_client: AsyncClient, app: FastAPI, file_storage: InMemoryFileStorage, monkeypatch
) -> None:
    put_calls = 0
...
async def test_download_content_sets_nosniff_header(authed_client: AsyncClient) -> None:
    ...
    response = await authed_client.get(f"/api/v1/files/{file_id}/content")
    assert response.status_code == 200
    assert response.headers["x-content-type-options"] == "nosniff"


async def test_upload_leaves_no_orphaned_blob_when_the_db_insert_fails(
    authed_client: AsyncClient, monkeypatch, file_storage: InMemoryFileStorage
) -> None:
    ...
    with pytest.raises(RuntimeError, match="db insert failed"):
        await _upload(authed_client, "notes.txt", b"hello world")
...
async def test_delete_returns_409_when_file_is_referenced(
    ...
    upload_response = await _upload(authed_client, "referenced.txt", b"data")
    file_id = upload_response.json()["id"]
...
async def test_ready_v1_checks_object_storage_healthcheck(
    ...
    response = await authed_client.get("/api/v1/ready")
