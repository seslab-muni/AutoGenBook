...
**Worker parallelism (issue #134):** a project holds one `running` run plus up to `MAX_QUEUED_RUNS_PER_PROJECT` `queued` ones (`api/application/runs.py:queue_admission_blocker`), and `SqlAlchemyRunQueue.claim`'s `SELECT ... FOR UPDATE SKIP LOCKED` lets multiple worker slots/replicas claim different projects' runs concurrently without double-claiming. The Kubernetes deployment (`k8s/worker.yaml`) runs 5 single-slot replicas (`replicas: 5`, `WORKER_CONCURRENCY=1` from `autogenbook-config`) rather than 1 replica with 5 slots, so a crash or an OOM from one oversized KB build/LuaLaTeX pass only ever takes down one in-flight run, and a rolling deploy drains one pod (one run) at a time. Compose achieves the same total concurrency either by raising `WORKER_CONCURRENCY` (one container, N async slots sharing its CPU/memory) or by leaving it at 1 and running `docker compose up --scale worker=N` (N containers, real process isolation, closer to the Kubernetes shape). Either way, housekeeping (`requeue_stale`, the stale/orphaned work-dir sweeps) is still meant to run once per poll cycle across the whole fleet, not once per replica - `api/worker/__main__.py:_run_housekeeping_sweeps` wraps it in a `pg_try_advisory_lock` (Postgres only; a no-op under sqlite) on a fixed key, so only whichever replica's slot 0 wins the lock that cycle actually sweeps.
